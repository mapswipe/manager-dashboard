...
# hardened with two conventions the stock script lacks:
#   1. sed replacement metacharacters are escaped so values substitute literally
#      (APP_MAPILLARY_API_KEY is a `MLY|...|...` token — the stock script fed the
#      raw `|` into `sed s|…|…|` and the container exited on startup);
#   2. unfilled placeholders are resolved to JS `undefined` (quoted JS markers
#      only) and every leftover is warned about on stderr, instead of leaking the
#      literal marker — a truthy string — into the bundle (Sentry initialising
#      with a bogus DSN, Firebase with a bogus API key).
